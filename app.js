...
const unitList=["2C","6W","6F","FB","5C","1P","2P","3P","4P","6P","9P","2F","1F","9F","3W","9W","3WC","6WC"];
let previousDates=[];
...
function doExportPDF() {
  ...
  if(certPage.style.display==='flex'){
    pdf.addPage(); 
    const topMargin = 40;  // ← adjust this to push content down
    let y = topMargin;

    // ✅ Default company address (EDIT THIS ANYTIME)
    const DEFAULT_COMPANY_ADDRESS = `ANDERSON FIRE
Unit 7
Montrose Business Centre
Broomfield Industrial Estate
Montrose, Angus
DD10 8SY
CERTIFICATE OF MAINTENANCE - FIRE EQUIPMENT`;
...
pdf.setFontSize(20); // 👈 bigger size
pdf.text(compLines[0], pageWidth/2, y, {align:'center'});
y += 8;

// बाकी lines (normal size)
pdf.setFontSize(10);
